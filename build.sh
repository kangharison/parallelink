...
if [[ ! -f "${FIO_DIR}/Makefile" ]] \
   ...
   || [[ ! -f "${NVME_CLI_DIR}/meson.build" ]]; then
    echo "==> [0/5] git submodule update --init --recursive"
...
# ------------------------------------------------------------------
echo "==> [2/5] Build libnvm (BaM) userspace + kernel module"
mkdir -p "${BAM_BUILD}"
...
# ------------------------------------------------------------------
echo "==> [5/5] Collect artifacts → ${DIST}"
rm -rf "${DIST}"
mkdir -p "${DIST}"
